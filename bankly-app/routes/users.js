...
const { schemaUserPatch } = require("../schemas/userPatch");
...
router.patch(
  ...
  // FIXES BUG #4
  requireCorrectUser,
  // Previous version containing BUG #4:
  // requireAdmin,
  async function (req, res, next) {
    try {
      // Previous version containing BUG #4
      ...
      // FIXES BUG #5
      const { error, value } = schemaUserPatch.validate(fields);
      if (error) throw new ExpressError("Incorrect Schema", 401);
...
router.delete(
  ...
  async function (req, res, next) {
    try {
      ...
      // Previous version containing BUG #6
