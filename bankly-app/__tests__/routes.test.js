...
describe("GET /users", function () {
  ...
  // TESTS BUG #2
  test("should list only basic info", async function () {
    const expected = {
...
describe("GET /users/[username]", function () {
  ...
  // TESTS BUG #3
  test("should return 404 err if user cannot be found", async function () {
    const response = await request(app)
      .get("/users/u234131")
      .send({ _token: tokens.u1 });
...
describe("PATCH /users/[username]", function () {
  // TESTS BUG #1
  test("should deny invalid token", async function () {
    const response = await request(app).patch("/users/u1").send({
      _token:
        "eyJhbGessOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InUyIiwiYWRtaW4iOmZhbHNlLCJpYXQiOjE3MDI3ODUwMDN9.wr9W2G3FUsU1Mf3U2xy-LL_etTBF2PZv0Hm5Rvz_V70",
    }); //invalid token
  ...
  // TESTS BUG #4
  test("should patch data if user themself", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn4" });
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn4",
  ...
  test("should disallow patching not-allowed-fields: admin", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, admin: true });
    expect(response.statusCode).toBe(401);
    expect(response.body.message).toEqual("Incorrect Schema");
  });
  // TESTS BUG #5

  test("should disallow patching not-allowed-fields: username", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, username: "asfsaf;slkj;" });
    expect(response.statusCode).toBe(401);
    expect(response.body.message).toEqual("Incorrect Schema");
  });
  // TESTS BUG #5

  test("should disallow patching not-allowed-fields: password ", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, password: "1434akl;" });
    expect(response.statusCode).toBe(401);
    expect(response.body.message).toEqual("Incorrect Schema");
  });
